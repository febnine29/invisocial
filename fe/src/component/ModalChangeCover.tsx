...
export default function ModalChangeCover({onOpenCover, isOpenCover, onCloseCover, userid}:IProp){
  ...
  const updateCover = async (url: string | undefined) => {
    try {
      const res = await axios.post(`http://localhost:5000/api/v1/auth/updateCoverUserid=${userid}`, {coverPic: url})
      dispatch(getUserInfo(userid))
  ...
  const handleUpPost = async () => {
    setLoading(true)
    await uploadFiles();
    updateCover(downloadUrl[0])
    console.log(downloadUrl[0]);
    
    closeModal()
  }
    return (
    <Modal closeOnOverlayClick={false} isCentered isOpen={isOpenCover} onClose={onCloseCover}>
        <ModalOverlay />
        <ModalContent maxW={500}>
        <ModalHeader textAlign='center'>Change your cover picture</ModalHeader>
        <ModalCloseButton />
